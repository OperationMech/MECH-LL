            ...
            while(i < localCode.length) {
                if(localCode[i].match("[^a-zA-Z0-9]|[a-z]|[0-9]|")){
                    if(localCode[i].match("[a-z]")){
                        ...
                        Tokens.push(new Token(["T_Char", localCode[i]],curLine,curCol));
                        OutputArea.value = OutputArea.value+"Found token: "+localCode[i]+"\n";
                    } else if(localCode[i] === "{" && !inStrMode) {
                        Tokens.push(new Token(["T_LCBrace", localCode[i]],curLine,curCol));
                        OutputArea.value = OutputArea.value + "Found token: "+ localCode[i] + "\n";
                    } else if(localCode[i] === "}" && !inStrMode) {
                        Tokens.push(new Token(["T_RCBrace", localCode[i]],curLine,curCol));
                        OutputArea.value = OutputArea.value + "Found token: "+ localCode[i] + "\n";
                    } else if(localCode[i] === "(" && !inStrMode) {
                        Tokens.push(new Token(["T_LParen", localCode[i]],curLine,curCol));
                        OutputArea.value = OutputArea.value + "Found token: "+ localCode[i]+"\n";
                    } else if(localCode[i] === ")" && !inStrMode) {
                        Tokens.push(new Token(["T_RParen", localCode[i]],curLine,curCol));
                        OutputArea.value = OutputArea.value +"Found token: "+localCode[i]+"\n";
                    } else if(localCode[i] === "=" && !inStrMode) {
                        if (localCode[i+1] === "="){
                            Tokens.push(new Token(["T_BoolOP", localCode[i] + localCode[i+1]], curLine, curCol));
                            OutputArea.value = OutputArea.value+"Found token: "+localCode[i]+localCode[i+1]+"\n";
                            curCol++;
                            i++;
                        } else {
                            Tokens.push(new Token(["T_AssignOP", localCode[i]], curLine, curCol));
                            OutputArea.value = OutputArea.value+"Found token: "+localCode[i]+"\n";
                        }
                        keyWord = "";
                    } else if(localCode[i] === "!" && !inStrMode) {
                        ...
                        if(inStrMode){
                            Tokens.push(new Token(["T_Space", localCode[i]], curLine, curCol));
                        ...
                        Tokens.push(new Token(["T_Quote", localCode[i]], curLine, curCol));
                        OutputArea.value = OutputArea.value+"Found token: "+localCode[i]+"\n";
                        inStrMode = !inStrMode;
                    } else if(localCode[i] === "+" && !inStrMode) {
                        Tokens.push(new Token(["T_IntOP", localCode[i]], curLine, curCol));
                        OutputArea.value = OutputArea.value+"Found token: "+localCode[i]+"\n";
                    } else if(localCode[i].match("[0-9]") && !inStrMode) {
                        Tokens.push(new Token(["T_Digit", localCode[i]], curLine, curCol));
                        OutputArea.value = OutputArea.value+"Found token: "+localCode[i]+"\n";
                    } else if(localCode[i] === "$" && !inStrMode) {
                        if(i < localCode.length-1 ){
                            Tokens.push(new Token(["T_EOF", localCode[i]], curLine, curCol));
                            OutputArea.value = OutputArea.value + "Found token: " + localCode[i] + "\n";
                    ...
                    if(keyWord === "print" || keyWord === "while" || keyWord === "if" || keyWord === "int" ||
                       keyWord === "boolean" || keyWord === "string" || keyWord === "true" || keyWord === "false") {
                        ...
                        if(keyWord === "true" || keyWord === "false") {
                            Tokens.push(new Token(["T_Boolval", keyWord], curLine, curCol));
                            OutputArea.value = OutputArea.value + "T_Boolval\n";
                        } else if(keyWord === "int" || keyWord === "boolean" || keyWord === "string") {
                            Tokens.push(new Token(["T_Type", keyWord], curLine, curCol));
                            OutputArea.value = OutputArea.value + "T_Type"+ "\n";
                        } else {
                            Tokens.push(new Token(["T_Kwd" + keyWord, keyWord], curLine, curCol));
